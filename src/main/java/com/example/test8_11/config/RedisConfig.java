...
public class RedisConfig {
    ...
    /**
     * 只要配置配好 能读取到属性  自动装配:自动创建对象 并且放入spring容器restTemplate
     ...
     * jdbctemplate
     * spring-data-redis
     * RedisConnectionFactory：springboot根据我们的配置进行初始化
     */
    @Bean
    ...
    /**
     * 为什么要序列化？redistemplate.set("a","abc")"a".getBytes
     * 序列化能加快网络传输，而且redis的value都是字符串，
     *redis 无论是哪种数据类型 里面都是放的字符串 set 里面也是放的字符串
